...
data = read.csv("/Users/Luis/Documents/GitHub/Computational-Investment/Tabelas/Datas.csv") # Pegar Datas
data = matrix(data[,1])
...
for (k in 1:n)
{
  for (i in 1:20)     # Ler arquivos XLSX
  {
    ...
    Temp = matrix(Temp[,2])
    if (nrow(Temp) < 8434)       # Para pegar o dado mesmo que esteja com problemas
    {
      Temp = matrix(,8434,)
      Temp[1] = Temp1
...
for (i in 1:ncol(Ativos))        # Trocar "NA" por ""
{
  for (k in 1:nrow(Ativos))
  {
    if(nchar(Ativos[k,i]) == 2)
